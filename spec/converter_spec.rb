  ...
  RSpec.describe ScaleConverterToKelvin do
    describe 'convert to KELVIN' do
      context 'from CELSIUS' do
        subject { ScaleConverterToKelvin.new.convert(0, CELSIUS) }
        it { is_expected.to eq(273.15) }
      end
      context 'from FAHRENHEIT' do
        subject { ScaleConverterToKelvin.new.convert(0, FAHRENHEIT) }
        it { is_expected.to eq(255.37) }
      end
      context 'from KELVIN' do
        subject { ScaleConverterToKelvin.new.convert(10, KELVIN) }
  ...
  RSpec.describe ScaleConverterFromKelvin do
    describe 'convert from KELVIN' do
      context 'to CELSIUS' do
        subject { ScaleConverterFromKelvin.new.convert(273.15, CELSIUS) }
        it { is_expected.to eq(0) }
      end
      context 'to FAHRENHEIT' do
        subject { ScaleConverterFromKelvin.new.convert(255.37, FAHRENHEIT) }
        it { is_expected.to eq(0) }
      end
      context 'to KELVIN' do
        subject { ScaleConverterFromKelvin.new.convert(10, KELVIN) }
        it { is_expected.to eq(10) }
      end
    end
  end
